...
import { isValidPhone, isValidName, isValidEmail, extractError, shakeElement, formatMobileDisplay } from '../utils/helpers'
...
const staggerWrap = { animate: { transition: { staggerChildren: 0.065 } } }
...
export default function Register() {
  ...
  const validate = () => {
    const errs = {}
    if (!isValidName(firstName)) errs.firstName = 'First name must be at least 2 characters'
    if (!isValidName(lastName))  errs.lastName  = 'Last name must be at least 2 characters'
    if (email && !isValidEmail(email)) errs.email = 'Enter a valid email address'
    if (!isValidPhone(phone))    errs.phone = 'Enter a valid 10-digit mobile number'
    setErrors(errs)
    if (Object.keys(errs).length) { shakeElement(cardRef); return false }
    return true
  }
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      if (err?.response?.status === 409)
        setApiErr('This number is already registered. Sign in instead.')
      else
        ...
        <motion.div variants={staggerWrap} initial="initial" animate="animate">
          ...
          <motion.div variants={fadeItem} className="mb-6">
            ...
            <p className="text-[0.875rem] text-muted font-light leading-relaxed">
              Create your account and start your AI food journey.
            </p>
          </motion.div>

          {/* Social login */}
          <motion.div variants={fadeItem} className="grid grid-cols-2 gap-2.5">
            <SocialLoginButton icon={<GoogleIcon/>} label="Google" onClick={() => console.log('Google OAuth')}/>
            <SocialLoginButton icon={<AppleIcon/>}  label="Apple"  onClick={() => console.log('Apple OAuth')}/>
          </motion.div>

          <motion.div variants={fadeItem} className="ch-divider">or with details</motion.div>

          {/* Name row */}
          <motion.div variants={fadeItem} className="grid grid-cols-2 gap-2.5 mb-3.5">
            <InputField
              label="First Name"
          ...
          {/* Email */}
          <motion.div variants={fadeItem} className="mb-3.5">
            <InputField
          ...
          {/* Phone */}
          <motion.div variants={fadeItem} className="mb-1">
            <PhoneInput
              value={phone}
          ...
          <motion.p variants={fadeItem} className="text-center text-[0.8rem] text-muted mt-5">
            ...
            <Link to="/auth/login" className="font-semibold hover:underline" style={{color:'#F26522'}}>
              Sign in
            </Link>
          </motion.p>
